...
function getCoreGlobals(ctx) {
  ...
  return [`<script nonce="${nonce}">`, `__ROUTE_PREFIX__ = ${JSON.stringify(ctx.prefix)};`, // consumed by ./client
...
function serverApp () {
  ...
  return class ServerApp extends FusionApp {
    ...
    callback() {
      this.resolve(); // $FlowFixMe

      this._app.use(compose(this.plugins)); // $FlowFixMe
